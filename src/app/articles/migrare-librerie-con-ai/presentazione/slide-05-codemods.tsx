    ...
    <div className="flex flex-col h-full py-6 px-6">
      ...
      <div className="flex-1 min-h-0 flex items-center justify-center">
        <svg viewBox="0 0 520 780" className="w-full max-w-2xl h-full">
          ...
          {edges.map((edge, i) => {
            ...
            return (
              <g key={`${edge.from}-${edge.to}`}>
                <motion.line
                  x1={x} y1={y1} x2={x} y2={y2 - 4}
                  stroke="var(--pres-muted)" strokeWidth="1.5"
                ...
                {edge.label && (
                  <motion.text
                    x={x + 6} y={(y1 + y2) / 2} fill="var(--pres-success)"
                    fontSize="11" fontWeight="600"
                    initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    transition={{ delay: 0.9 + i * 0.15 }}
                  >{edge.label}</motion.text>
                )}
              </g>
            );
          })}
          ...
          />
          <ArrowTip x={300} y={60} angle={180} color="var(--pres-danger)" delay={1.4} />
          <motion.text
            x="410" y="170" fill="var(--pres-danger)" fontSize="11" fontWeight="600"
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.4 }}
          >no → rianalizza</motion.text>
          ...
          />
          <ArrowTip x={300} y={160} angle={180} color="var(--pres-danger)" delay={1.5} />
          <motion.text
            x="450" y="390" fill="var(--pres-danger)" fontSize="11" fontWeight="600"
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }}
          >no → diagnostica</motion.text>
          <motion.text
            x="450" y="404" fill="var(--pres-danger)" fontSize="11" fontWeight="600"
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5 }}
          >prima</motion.text>
          ...
          />
          <ArrowTip x={300} y={60} angle={180} color="var(--pres-accent)" delay={1.8} />
          <motion.text
            x="430" y="748" fill="var(--pres-accent)" fontSize="11" fontStyle="italic"
            initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.8 }}
          >batch successivo</motion.text>
        </svg>
      </div>
    </div>
  );
