...
const pillars = [
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" width="22" height="22" fill="none">
        ...
        <path d="M12 2v3M12 19v3M2 12h3M19 12h3M5.6 5.6l2.1 2.1M16.3 16.3l2.1 2.1M5.6 18.4l2.1-2.1M16.3 7.7l2.1-2.1" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
      </svg>
    ),
  },
  {
  ...
  },
  {
    ...
    icon: (
      <svg viewBox="0 0 24 24" width="22" height="22" fill="none">
        <path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
      </svg>
    ),
  },
  {
      ...
      <div className="flex flex-col lg:flex-row items-center gap-8 w-full max-w-4xl">
            ...
            {pillars.map((p, i) => {
              ...
              return (
                ...
                >
                  <circle
                    cx={nx} cy={ny} r={22}
                    fill={`rgba(${hexToRgb(p.color)}, 0.12)`}
                    stroke={p.color}
                    strokeWidth="1.5"
                  />
                  <text
        ...
        <div className="flex flex-col gap-3 w-full">
          {pillars.map((p, i) => (
            <motion.div
              key={p.id}
              className="flex items-start gap-3 rounded-lg border border-[#334155] bg-[rgba(255,255,255,0.02)] px-4 py-3"
              initial={{ opacity: 0, x: 20 }}
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.4, delay: 0.8 + i * 0.12 }}
            >
              <span style={{ color: p.color }} className="mt-0.5 flex-shrink-0">
                {p.icon}
              </span>
              <div>
                <p className="font-semibold text-sm" style={{ color: p.color }}>
                  {p.label} <span className="text-[#94a3b8] font-normal">— {p.role}</span>
                </p>
                <p className="text-[#94a3b8] text-xs mt-0.5">{p.desc}</p>
              </div>
            </motion.div>
          ))}
        </div>
