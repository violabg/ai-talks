...
Esempio di prompt efficace per migrare un pattern di state management:

```text
Contesto:
- Stiamo migrando da Zustand v4 a Zustand v5
- Breaking change: `set` non supporta più il merge implicito, bisogna usare `set((state) => ({ ...state, campo: valore }))`

Pattern vecchio (v4):
  store.set({ utente: nuovoUtente })

Pattern nuovo (v5):
  store.set((state) => ({ ...state, utente: nuovoUtente }))
...
**Non mischiare trasformazioni diverse nello stesso prompt.** Se devi aggiornare sia le API di state sia i tipi, fallo in due passi separati. Ogni passo deve avere un unico obiettivo verificabile.
